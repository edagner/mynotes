...
#convert scikit dataset into pandas
def answer_one():
...
cancerdf.groupby(['target']).size()

#class distribution of malignant vs benign
def answer_two():
    cancerdf = answer_one()
    tumorCounts = [len(cancerdf[cancerdf['target']==0]),len(cancerdf[cancerdf['target']==1])]
    s = pd.Series(tumorCounts, index=['malignant', 'benign'])
    return s
...
def answer_eight():
    ...
    cancer_accuracy = knn.score(X_test, y_test)
    
    return cancer_accuracy
...
#fits training data really well(1.0) but scores terribly on test data
...
#scores bad on both training and test data
...
#higher value of C means less regularization
